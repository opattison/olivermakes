...
<figure class="image--wide">
  <img
    src="{{ page.image[1].src | imgix_url: w: 720, q: 60 }}"
    sizes="{{ site.sizes }}"
    srcset="{% for source in site.srcset %}{{ page.image[1].src | imgix_url: w: site.srcset[forloop.index0], q: 60 }} {{ site.srcset[forloop.index0] }}w{% if forloop.last == false %}, {% endif %}{% endfor %}"
